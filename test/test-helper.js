...
describe('Yeoman generator - helper', function () {
	...
	it('create helper in folder without test', function (done) {
		var helper = helpers.createGenerator('footguard:helper', ['../../helper'], ['foo', 'boo']);
		...
		helper.run([], function() {
			helpers.assertFiles([
				['src/coffee/app/helpers/boo/foo_helper.coffee', /Foo = \(\)->/]
			]);
			done();
		});
	});
	...
	it('create helper in folder with test', function (done) {
		...
		helper.run([], function() {
			helpers.assertFiles([
				['src/coffee/app/helpers/boo/foo_helper.coffee', /Foo = \(\)->/],
				['src/coffee/spec/unit/helpers/boo/foo_helper_spec.coffee', /Test Foo Helper/],
				['src/coffee/spec/unit/helpers/boo/foo_helper_spec.coffee', /define \['app\/helpers\/boo\/foo_helper'\], \(Foo\)->/],
				['src/coffee/spec/unit/all_unit_tests.coffee', /"spec\/unit\/helpers\/boo\/foo_helper_spec"/]
			]);
			done();
		});
	});
